...
public class WebSocket {
			...
			public void handleOpen(@PathParam("userId") String userId, Session session) throws IOException {
						...
						List<Session> mapList = slMap.get(userId); // 같은 userId의 session 리스트 호출
						if (mapList == null || mapList.size() == 0) { // 없다면 생성
							mapList = new ArrayList<>();
						}
						mapList.add(session); // 생성된or호출된 session리스트에 add
						...
						//if (runTask == null) { // 없다면 생성
							...
							runTask = new TimerTask() {
								...
								public void run() {
									...
									String message = userId + "알림 [" + System.currentTimeMillis() + "]";
									Iterator<Session> itr = slMap.get(userId).iterator();
									while (itr.hasNext()) {
										try {
											Session session = itr.next();
											System.out.println("while문 도는 중");
							...
							timer.scheduleAtFixedRate(runTask, 0, 3000); // 3초마다 할 tast
						...
						if(message.substring(0, 4).equals("PUSH")) {
							...
							for (Map.Entry<String, List<Session>> entry : slMap.entrySet()) {
								...
								if (entry.getKey().equals(writer)) {
									for (Session se : entry.getValue()) {
										se.getBasicRemote().sendText(boardNo+"번 게시글에 댓글이 달렸습니다.");
									}
								}
							}
			...
			public void handleClose(@PathParam("userId") String userId, Session session) throws IOException {
						...
						for (Map.Entry<String, List<Session>> entry : slMap.entrySet()) {
							...
							if (entry.getKey().equals(userId)) {
								...
								for (Session se : entry.getValue()) {
									se.getBasicRemote().sendText("띠요옹 " + userId + "님이 퇴장하셨습니다.");
								}
							}
						}
						/*
